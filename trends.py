...
def analyze_tweet_sentiment(tweet):
    ...
    sentiment_list = list(map(sentiment_value, (filter(has_sentiment, (map(get_word_sentiment, text))))))
    if len(sentiment_list) != 0:
        average = make_sentiment(sum(sentiment_list) / len(sentiment_list))
    return average
...
def find_centroid(polygon):
    ...
    while counter_area <= n - 2:
        area += latitude(polygon[counter_area])*longitude(polygon[counter_area+1]) - latitude(polygon[counter_area+1])*longitude(polygon[counter_area])
        counter_area += 1
    area = (0.5)*area
    ...
    while counter_x <= n - 2:
        x += (latitude(polygon[counter_x]) + latitude(polygon[counter_x+1]))*(latitude(polygon[counter_x])*longitude(polygon[counter_x+1]) - latitude(polygon[counter_x+1])*longitude(polygon[counter_x]))
        counter_x += 1
    x = (1/(6*area))*x
    ...
    while counter_y <= n - 2:
        y += (longitude(polygon[counter_y]) + longitude(polygon[counter_y+1]))*(latitude(polygon[counter_y])*longitude(polygon[counter_y+1]) - latitude(polygon[counter_y+1])*longitude(polygon[counter_y]))
        counter_y += 1
    y = (1/(6*area))*y
...
def find_state_center(polygons):
    ...
    for i in polygons:
        ...
        centroid_x += (centroid[0] * centroid[2])
        centroid_y += (centroid[1] * centroid[2])
        area_sum += centroid[2]

    return make_position(centroid_x / area_sum, centroid_y / area_sum)
...
def find_closest_state(tweet):
    """
    Finds the tweet's closest state
    """

    old = 10000000 

    for each_state in us_states:
        update = geo_distance(tweet_location(tweet), find_state_center(us_states[each_state]))
        if update < old:
            current_state = each_state 
            old = update
...
def group_tweets_by_state(tweets):
    ...
    for each_tweet in tweets:
        state = find_closest_state(each_tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        total_sentiment_state = 0
        number_of_tweets = 0
        average_sentiment_state = 0
        for each_tweet in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(each_tweet)) or analyze_tweet_sentiment(each_tweet) == 0:
                total_sentiment_state += sentiment_value(analyze_tweet_sentiment(each_tweet))
                number_of_tweets += 1
        if number_of_tweets != 0:
            average_sentiment_state = total_sentiment_state / number_of_tweets
            averaged_state_sentiments[state] = average_sentiment_state
